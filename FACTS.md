...
|---|---|---|---|---|
| offline suite (Stage 10, `redteam/crazy-testing` branch) | 565 pass / 1 skip / 2 xfail (= 538 branch baseline + 27 Stage-10 add-only; **pre==post app edits ⇒ 0 regressions**). NB: 538 ≠ the main/Stage-8 373 — the branch adds `tests/test_redteam.py` (167 + 2 strict xfails characterizing the lexical-grounding limit) | `make test` | 2026-06-27 | fe453f4 |
| query refinement (Stage 10, `QREF*`/`DRAFT-COT*`) | MockLLM identity ⇒ offline retrieval byte-identical ⇒ `make eval` metrics UNCHANGED (recall 1.0 / raw_grounded 0.833 / routing 1.0 / review{u1}); `refine_query` audited per item (`original==optimized` offline); **defect fixed**: `item.question in ClaudeLLM._build_prompt` = True; `<thinking>` stripped before gate/export | `make test` + `make eval` + `make demo` | 2026-06-27 | fe453f4 |
