...
> reaching the draft prompt. Verified offline/deterministic (`tests/test_stage10_query_refinement.py`,
> 31 tests as of 2026-06-28). The refine-query `<thinking>` self-checks are **defense-in-depth UX, not
> enforcement** — the `RULE_*` chokepoints stay code-enforced (CLAUDE.md §5). Cross-references the Stage 3
> modules (`context_stack.py`, `llm.py`, `draft.py`) without retro-editing the ✅ Stage 3/4 DoD.
>
> **Revision 2026-06-28 (two-key authorized):** the **draft** `<thinking>` scaffold was REMOVED (live
> evidence: it suppressed inline citations + tanked live grounding). `DRAFT-COT1`'s thinking-directive
> assertion was retired and replaced by a positive assertion of the new draft contract (inline
> `[chunk_id]` citations + answer-only); `DRAFT-COT2` keeps the **defensive** strip in `ClaudeLLM.draft`.

| ID | Gate | How it is checked |
|---|---|---|
| `QREF1` | `<thinking>` strip is deterministic & total | `strip_thinking_block` (depth-aware scan) removes well-formed, dangling-open, case-insensitive, multiple, **and nested** blocks; preserves token boundaries (no fusion); empty remainder → caller fallback (`TestStripThinkingBlock`) |
...
| `QREF3` | Pipeline injects + audits the stage | `run_pipeline` emits one `refine_query` `tool_call` audit event per item with `original` + `optimized` + `to_state`; offline `optimized == original`; a rewriting provider's optimized query is the one audited (`TestPipelineRefinement`) |
| `DRAFT-COT1` | Original question reaches the draft prompt; draft contract = inline citations, answer-only | `assemble_context(...).question == item.question`; `item.question in ClaudeLLM()._build_prompt(stack)` (the **defect regression**); the prompt requests inline `[chunk_id]` citations + answer-only and carries **no** `<thinking>` directive (2026-06-28 spec change); pre-Stage-10 `ContextStack(...)` still validates (default `""`) (`TestContextStackQuestion`) |
| `DRAFT-COT2` | Draft defensively strips `<thinking>` before gate/export | with only the Anthropic client faked (real `_build_prompt`/`draft`/strip path), a `<thinking>…</thinking> answer [chunk_id]` response yields `DraftAnswer.text` with no `<thinking>` and a parsed citation; a thinking-only response degrades to `UNGROUNDED_PLACEHOLDER`; `MockLLM` never emits `<thinking>`; the **refine** directive constant retains its scaffold (`TestDraftThinkingStrip`, `TestPromptScaffoldConstants`) |
