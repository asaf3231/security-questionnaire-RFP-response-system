...
RULE_NO_FABRICATED_METRIC: every value is derived from compute_ functions applied to
  labeled fixtures; no number is hardcoded.
...
from app.schema import DraftAnswer, Citation, ConfidenceResult, QuestionnaireItem, RetrievedChunk
...
# ---------------------------------------------------------------------------
# Per-case evaluation helpers (all computed — never hardcoded)
# ---------------------------------------------------------------------------

def _simulate_grounding(chunks: list[RetrievedChunk]) -> GroundingResult:
    """Simulate a grounded draft for eval purposes.

    Uses the first retrieved chunk as the grounding source.  Returns a
    GroundingResult(grounded=True) when chunks are non-empty, False otherwise.
    This is a controlled approximation for eval harness purposes:
    the metric that matters is routing_accuracy (based on routing decisions)
    and calibration (confidence band vs grounded outcome).  Grounding rate is
    computed against expected_grounded labels.

    RULE_NO_FABRICATED_METRIC: the grounded boolean is derived from retrieval
    results, not hardcoded.
    """
    if not chunks:
        # No retrieved evidence → cannot ground
        return GroundingResult(
            grounded=False,
            answer=DraftAnswer(
                text="[NO GROUNDED ANSWER FOUND — ROUTED FOR HUMAN INPUT]",
                citations=[],
            ),
            reason_code="GROUNDING_FAIL",
        )
    # Use the top chunk as the citation source
    top = chunks[0]
    return GroundingResult(
        grounded=True,
        answer=DraftAnswer(
            text=top.answer,
            citations=[Citation(chunk_id=top.chunk_id)],
        ),
        reason_code=None,
    )
...
def run_eval(
    *,
    ...
    provider:
        Optional LLM provider (unused in the offline harness — grounding is
        simulated deterministically from retrieval).  Reserved for future extension.
    policy_tags:
    ...
    for case in eval_cases:
        ...
        # Simulate grounding deterministically (RULE_NO_FABRICATED_METRIC)
        grounding = _simulate_grounding(chunks)
        actual_grounded: bool = grounding.grounded

        # Grounding rate: does the simulated grounding match the label?
